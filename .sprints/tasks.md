...
| 8 | Implement hydrate-on-tenant-load — when `tenants.load(tenantId)` resolves, trigger Phase 1 hydrate: if cloud adapter present, attempt `hydrateFromCloud` with tenant's `cloudMeta`; on cloud failure emit `cloud-unreachable` event and fall back to `hydrateFromLocal`; start sync scheduler after hydrate completes | E24 | developer | done | plan | 2026-03-24T00:00:00Z | 2026-03-24T06:05:00Z |
| 9 | Implement `strata.sync()` — delegate to `syncNow()` via sync lock using active tenant's `cloudMeta`; reject with descriptive error if no tenant loaded or no cloud adapter configured; return `Promise<SyncResult>` | E24 | developer | done | plan | 2026-03-24T00:00:00Z | 2026-03-24T06:05:00Z |
