...
import type { Strata } from '../../../src/index.js';
...
// ── Tests ───────────────────────────────────────────────────────────
describe('Integration: createStrata (Sprint 007)', () => {
  ...
  describe('tenant switching via strata.load()', () => {
    ...
    it('isolates data between tenants', async () => {
      ...
      // Switch to tenant B — should see no data
  ...
  // ── 7. End-to-end: define → createStrata → tenant → repo → save → sync → verify
  describe('end-to-end workflow', () => {
    it('full lifecycle: define entities → create strata → create tenant → load → save → sync → verify → dispose', async () => {
      ...
      // Step 2: Create adapter and strata
      const adapter = makeAdapter();
      const strata = track(
        createStrata({
      ...
      const memId2 = await memberRepo.save({ email: 'bob@acme.com', role: 'member' });
      ...
      // Step 8: Trigger sync (should not throw)
      expect(() => strata.sync()).not.toThrow();
    ...
    it('multi-tenant end-to-end: two tenants with isolated data', async () => {
      ...
      // Switch to tenant 2 — should see no widgets
