...
import type { Strata } from '../../../src/strata/index.js';
import type { TenantManager } from '../../../src/tenant/index.js';
...
import { StrataProvider } from '../../../src/react/strata-context.js';
import { TenantProvider } from '../../../src/react/tenant-context.js';
import { useRepo } from '../../../src/react/use-repo.js';
import { useTenant } from '../../../src/react/use-tenant.js';
import { withObservable } from '../../../src/react/with-observable.js';
import { withCollection } from '../../../src/react/with-collection.js';
import { TenantPicker } from '../../../src/react/tenant-picker.js';
import { TenantCreationWizard } from '../../../src/react/tenant-creation-wizard.js';
...
// 1. StrataProvider
// ═══════════════════════════════════════════════════════════════════
describe('Integration: StrataProvider', () => {
...
// 3. useTenant
// ═══════════════════════════════════════════════════════════════════
describe('Integration: useTenant', () => {
...
// ═══════════════════════════════════════════════════════════════════
describe('Integration: withObservable HOC', () => {
...
// ═══════════════════════════════════════════════════════════════════
describe('Integration: withCollection HOC', () => {
...
// 6. TenantPicker
// ═══════════════════════════════════════════════════════════════════
describe('Integration: TenantPicker', () => {
...
// ═══════════════════════════════════════════════════════════════════
describe('Integration: TenantCreationWizard', () => {
...
// ═══════════════════════════════════════════════════════════════════
// 8. End-to-end: StrataProvider → TenantProvider → useRepo →
//    save → observe updates in UI
// ═══════════════════════════════════════════════════════════════════
describe('Integration: End-to-end React flow', () => {
  it('renders a component tree with StrataProvider → TenantProvider → useRepo, saves an entity and observes UI updates', async () => {
    ...
    const mockRepo: Repository<Todo> = {
      ...
      save: vi.fn().mockImplementation(async (entity: Todo) => {
        // Simulate save: push into the subject
    ...
    // App component that uses useRepo and observeAll
    function TodoApp() {
      ...
      const EnhancedList = withCollection<Todo>(TodoListView, () => subject);
    ...
    // Wait for tenant loading to finish
    await waitFor(() => {
      expect(screen.getByTestId('todo-count').textContent).toBe('0');
    });

    // Click add button to save a todo
    fireEvent.click(screen.getByTestId('add-todo'));

    // The subject should have been updated by our mock save
    await waitFor(() => {
    ...
    // Simulate another entity appearing via observable
    act(() => {
      todoSubject.next([
  ...
  it('combines TenantPicker and TenantCreationWizard in a full app shell', async () => {
    ...
    (tm.list as ReturnType<typeof vi.fn>)
      .mockResolvedValueOnce([tenantA])        // initial load
      .mockResolvedValueOnce([tenantA, created]); // after create
    ...
    // Wait for initial tenant list to load
    await waitFor(() => {
      expect(screen.getByTestId('tenant-picker')).toBeDefined();
    });

    // Create a new tenant via wizard
