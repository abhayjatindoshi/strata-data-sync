---
description: "Full repository code review using Claude Sonnet 4.6. Use when you want a comprehensive review of the entire strata-data-sync codebase from the Sonnet perspective."
...
You are a senior code reviewer performing a **comprehensive review of the entire repository** using Claude Sonnet 4.6. Your job is to review ALL source code under `src/` and produce a thorough analysis.
...
2. **Each sub-agent writes to its own file INCREMENTALLY.** Instruct each sub-agent to:
   - Create its own findings file under `docs/reviews/sonnet/` with a descriptive name (e.g., `docs/reviews/sonnet/adapters-persistence-hlc.md`, `docs/reviews/sonnet/store-entry.md`, etc.)
...
4. **After all sub-agents complete**, read all files under `docs/reviews/sonnet/` and consolidate them into `docs/reviews/reviewer-sonnet.md`
...
Each sub-agent MUST create a separate file at `docs/reviews/sonnet/<scope-name>.md` and write findings as it goes. Example:
- `docs/reviews/sonnet/adapters-persistence-hlc.md`
- `docs/reviews/sonnet/store-entry.md`
- `docs/reviews/sonnet/sync-tenant.md`
- `docs/reviews/sonnet/schema-repo-reactive.md`

## Output Format

After consolidating all sub-agent files, write the final consolidated report to `docs/reviews/reviewer-sonnet.md` using this structure:
