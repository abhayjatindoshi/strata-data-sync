...
| [Adapter Contract](docs/adapter.md) | `BlobAdapter` interface, `meta` per-call, transform pipeline |
| [Tenant System](docs/tenant.md) | Multi-tenancy, `meta`, tenant lifecycle, sharing, tenant list storage |
...
- **`meta` per-call** — adapters receive opaque tenant location info. No generics. No tenant type pollution.
